...
BEGIN;
	...
	BEGIN;
					...
					SELECT TOP(1)
						CASE
							...
							ELSE
								...
								(
									ms,
									1000 * (DATEPART(ms, DATEADD(second, -(r.total_elapsed_time / 1000), GETUTCDATE())) / 500) - DATEPART(ms, DATEADD(second, -(r.total_elapsed_time / 1000), GETUTCDATE())),
									DATEADD(second, -(r.total_elapsed_time / 1000), GETUTCDATE())
								)
						END AS start_time
					FROM sys.dm_exec_requests AS r
					WHERE
		...
		SET @sql =
			...
			(
				SELECT TOP(@i)
					y.*,
					CASE
						WHEN DATEDIFF(day, y.start_time, GETUTCDATE()) > 24 THEN
							DATEDIFF(second, GETUTCDATE(), y.start_time)
						ELSE DATEDIFF(ms, y.start_time, GETUTCDATE())
					END AS elapsed_time,
				...
				(
					SELECT TOP(@i)
						...
						(
							...
							(DATEADD(second, -(tx.ms_ticks / 1000), GETUTCDATE()))
						) AS start_time,
						sp.login_time,
						CASE
		...
		SET @last_collection_start = GETUTCDATE();
